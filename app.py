...
CRITICAL_ATTACK_PATTERNS = [
    ...
    r"\b(?:system\s+override|override\s+system|ignore\s+instructions|bypass\s+rules)\b",
    r"\b(?:instructions?\s+(?:to|for)\s+(?:exploit|hack|attack)\s+(?:system|server|website|database))"

    # -------- PROMPT INJECTION / JAILBREAK --------

r"\bignore\s+(?:all\s+)?(?:previous\s+)?instructions\b",
r"\bdisregard\s+(?:all\s+)?(?:rules|instructions|restrictions)\b",
r"\bforget\s+(?:the\s+)?(?:rules|instructions|safety)\b",
r"\boverride\s+(?:system|security|filters|instructions)\b",
r"\bbypass\s+(?:safety|security|filters|restrictions)\b",
r"\bsystem\s+override\b",
r"\bdeveloper\s+mode\b",

# -------- ROLE PLAY ATTACKS --------

r"\bact\s+as\s+(?:a\s+)?(?:hacker|attacker|cybercriminal)\b",
r"\bpretend\s+(?:to\s+be\s+)?(?:a\s+)?(?:hacker|attacker)\b",
r"\bsimulate\s+(?:being\s+)?(?:a\s+)?(?:hacker|attacker)\b",

# -------- HIDDEN INTENT --------

r"\bfor\s+(?:educational|research)\s+purposes\s+only.*(?:hack|malware|exploit)",
r"\bexplain\s+how\s+(?:hackers?|attackers?)\s+(?:steal|exploit|break)\b",

# -------- CYBER ATTACK ACTIONS --------

r"\b(?:hack|exploit|breach|compromise)\s+(?:system|server|database|network|account)\b",
r"\bbypass\s+(?:authentication|login|security|protection)\b",
r"\blaunch\s+(?:a\s+)?(?:ddos|dos)\s+attack\b",
r"\bperform\s+sql\s+injection\b",

# -------- DATA / SYSTEM PROMPT EXTRACTION --------

r"\breveal\s+(?:the\s+)?(?:system\s+prompt|internal\s+prompt)\b",
r"\bshow\s+(?:the\s+)?(?:hidden\s+instructions|developer\s+instructions)\b",
r"\bdisplay\s+(?:confidential|training\s+data)\b",
r"\bprint\s+(?:internal\s+policies|system\s+rules)\b",

# -------- MALWARE DEVELOPMENT --------

r"\b(?:create|write|build|generate)\s+(?:a\s+)?(?:virus|malware|ransomware|spyware|trojan)\b",
r"\bdevelop\s+(?:malware|ransomware|virus)\b",
]
